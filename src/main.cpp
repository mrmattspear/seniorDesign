...
//--------------wifi shield-----------------------------------------------
 ...
 void loop() {
   ...
   //----------Begin Current/Voltage Sensor Setup-----------------------
   float shuntvoltage = 0; // From library
   float busvoltage = 0;  // From libary
   float current_mA = 0 * 1000; //From library * 1000 to get Amps
   float loadvoltage = 0;       // From library

   shuntvoltage = ina219.getShuntVoltage_mV(); // From library
   busvoltage = ina219.getBusVoltage_V();     // From libary
   current_mA = ina219.getCurrent_mA();      // From library
   loadvoltage = busvoltage + (shuntvoltage / 1000);  // From library
...
//-------------End Current/Voltage Sensor Setup ------------------------


 switch(state){
   case idle:
   delay(60000);//delay every 60s
   state = sampleTemp;
   break;
